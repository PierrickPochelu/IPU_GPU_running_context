...
try:
      ...
      DEVICE, GBS, GRADAC, MODELID = "GPU", 4, 1, 7
...
(
    ...
    num_test_samples,
    raw_datasets,
)=get_glue_data("cola",micro_batch_size=context.local_batch_size, cache_dir="./tmp/")
tf_dataset_train = train_dataset
tf_dataset_test = test_dataset
...
with context.device_scope():  # Mapping between hardware and tensors
    ...
    train_steps_per_exec = int(num_train_samples.numpy() // context.local_batch_size)
    eval_steps_per_exec = int(num_test_samples.numpy()  // context.local_batch_size)


    print(train_steps_per_exec)
